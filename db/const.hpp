...
namespace ron {
    ...
    inline MAP uuid2map (const Uuid& u) {
        switch (uint8_t(u.value()._64>>52)) {
            case uint8_t(CHAIN_MAP_NAME>>52): 	return u==CHAIN_MAP_ID ? CHAIN_MAP : MAP_COUNT;
            case uint8_t(META_MAP_NAME>>52): 	return u==META_MAP_ID ? META_MAP : MAP_COUNT;
            case uint8_t(OP_MAP_NAME>>52): 	return u==OP_MAP_ID ? OP_MAP : MAP_COUNT;
            case uint8_t(YARN_MAP_NAME>>52): 	return u==YARN_MAP_ID ? YARN_MAP : MAP_COUNT;
            case uint8_t(SHA2_MAP_NAME>>52): 	return u==SHA2_MAP_ID ? SHA2_MAP : MAP_COUNT;
            case uint8_t(PREV_MAP_NAME>>52): 	return u==PREV_MAP_ID ? PREV_MAP : MAP_COUNT;
            case uint8_t(HEAD_MAP_NAME>>52): 	return u==HEAD_MAP_ID ? HEAD_MAP : MAP_COUNT;
            case uint8_t(OBJ_MAP_NAME>>52): 	return u==OBJ_MAP_ID ? OBJ_MAP : MAP_COUNT;
            case uint8_t(CSV_MAP_NAME>>52): 	return u==CSV_MAP_ID ? CSV_MAP : MAP_COUNT;
            case uint8_t(TXT_MAP_NAME>>52): 	return u==TXT_MAP_ID ? TXT_MAP : MAP_COUNT;
            case uint8_t(JSON_MAP_NAME>>52): 	return u==JSON_MAP_ID ? JSON_MAP : MAP_COUNT;
            default: return MAP_COUNT;
        }
    }
