...
struct ChainMapper {
    ...
    Status Map(Builder& response, Cursor& query,
               const VV& hili = EMPTY_VV) const;
...
struct MasterMapper {
    ...
    explicit MasterMapper(HostReplica* host)
        : chain_{host},
          op_{host},
          yarn_{host},
          meta_{host},
    ...
    Status Map(Builder& response, Cursor& query,
        ...
        if (ref.version() != NAME)
            return Status::BAD_STATE.comment("the ref is not a mapper id");
