...
Status MetaMapper<Frame>::Map(Builder& response, Cursor& query,
                              const VV& hili) const {
    ...
    switch (map) {
        ...
        case SHA2_MAP:
            ...
            break;
        case PREV_MAP:
            response.AppendNewOp(RAW, PREV_MAP_ID, ref,
                                 Uuid{meta.prev, ref.origin()});
            break;
        case HEAD_MAP:
            response.AppendNewOp(RAW, HEAD_MAP_ID, ref,
                                 Uuid{meta.chain, ref.origin()});
            break;
        case OBJ_MAP:
