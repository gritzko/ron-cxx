...
syn match NAME_ATOM />\=[0-9A-Za-z_~]\{1,10}/
...
syn match UUID_ATOM />\=[0-9A-Za-z_~]\{1,10}[-+$%][0-9A-Za-z_~]\{1,10}/
hi UUID_ATOM ctermfg=Green cterm=underline

syn match STRING_ATOM /'\(\\u[0-9a-fA-F]\{4}\|\\[^\n\r]\|[^'\\\n\r]\)*'/
hi STRING_ATOM ctermfg=DarkGreen

syn match OP_ID  /@[0-9A-Za-z_~]\{1,10}\([-+$%][0-9A-Za-z_~]\{1,10}\)\{0,1}/
hi OP_ID ctermfg=Blue

syn match REF_ID /:[0-9A-Za-z_~]\{1,10}\([-+$%][0-9A-Za-z_~]\{1,10}\)\{0,1}/
