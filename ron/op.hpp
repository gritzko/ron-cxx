...
struct Op {
  ...
  explicit Op(RON coding, TERM term) : atoms_{}, coding_{coding}, term_{term} {
  ...
  Op(const Uuid& id, const Uuid& ref, const Value& v0, const Value& v1)
      : Op{id, ref, v0} {
    atoms_.push_back(v1);
  }
  inline TERM term() const { return term_; }
  const Uuid& type() const { return (Uuid&)atoms_[0]; }
