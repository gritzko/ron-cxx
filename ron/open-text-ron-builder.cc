...
namespace ron {
    ...
    void OpenTextFrame::Builder::AddOp(const Op &op, const std::string &back_buf) {
        // TODO how do I know that the source frame is text, to avoid re-serialization?
    ...
    void OpenTextFrame::Builder::WriteInt(int64_t value) {
        char tmp[20];
        int len = sprintf(tmp, "%" PRId64, value);
        data_.append(tmp, (size_t)len);
    }

    void OpenTextFrame::Builder::WriteUuid(const Uuid& value) {
        // FIXME escaping
        data_.append(value.str());
    }

    void OpenTextFrame::Builder::WriteFloat(double value) {
        char tmp[20];
        int len = sprintf(tmp, "%le", value);
        data_.append(tmp, (size_t)len);
    }

    void OpenTextFrame::Builder::WriteString(const std::string& value) {
        std::string esc;
        OpenTextFrame::escape(esc, value);
        data_.append(esc);
    }

    void OpenTextFrame::escape (std::string& to, const char* buf, fsize_t size) {
        while (size) {
            switch (*buf) {
                case '\"':
                    to.push_back(OpenTextFrame::ESC);
                    to.push_back('"');
                    break;
                case '\'':
                    to.push_back(OpenTextFrame::ESC);
