...
/** Chain span metadata: head id, size, reference/payload. */
class Span {
    Uuid head_;
    fsize_t len_;
...
class Uuids {
   ...
   public:
    ...
    const Span& front_span() const { return spans_.front(); }
    const Span& back_span() const { return spans_.back(); }

    class const_iterator {
        Spans::const_iterator i_;
        fsize_t off_;
        const_iterator(Spans::const_iterator span, fsize_t offset)
            : i_{span}, off_{offset} {}
        friend class Uuids;
       ...
       public:
        ...
        inline void operator-=(fsize_t step) {}
        inline Uuid operator*() const { return i_->front().inc(off_); }
        /** for the cases of incrementally increasing payload value */
        inline fsize_t ref_inc() const { return i_->ref() + off_; }
        inline const Span& span() const { return *i_; }
        inline fsize_t span_offset() const { return off_; }
        inline bool operator==(const const_iterator& b) {
            return i_ == b.i_ && off_ == b.off_;
        }
    };

    const_iterator begin() const { return const_iterator{spans_.begin(), 0}; }

    const_iterator end() const { return const_iterator{spans_.end(), 0}; }
};
